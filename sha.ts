...
// Contoh penggunaan
async function main() {
  const githubUrl = "@https://github.com/bipproduction/wibu-storage.git";
  const branch = "main";
  
  const result = await getRepoCommitData(githubUrl, branch);
  console.log("\nHasil commits dari branch", branch + ":");
  console.log("Latest commit:", result.latest);
  console.log("\nSemua commits:", result.commits);
}
